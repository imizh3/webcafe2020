...
namespace WEB2020.Data
{
    public class ApplicationManage : BaseManage
    {
        ...
        public ApplicationManage(IConfiguration configuration, MARTDbContext _db) : base(configuration)
        {
            ...
            db = _db;
            Ngayphatsinh ngayphatsinh = db.Ngayphatsinhs.Where(d => d.Madonvi == this.Madonvi && d.Trangthai == 1).FirstOrDefault();
            PublicValue.ngayPhatSinh = ngayphatsinh != null ? ngayphatsinh.Ngayphatsinh1 : DateTime.Now;
        }

        public List<Dmptnx> getDmptnx()
        {
            return db.Dmptnxes.Where(db => db.Madonvi == this.Madonvi).ToList();
        }

        public List<Vat> GetVats()
        {
            return db.Vats.Where(d => d.Madonvi == this.Madonvi).ToList();
        }
        ...
        {
            return db.Thamsohethongs.Where(db => db.Madonvi == this.Madonvi).ToList();
        }
