...
namespace WEB2020.Data
{
    ...
    public class GiaodichManage : BaseManage
    {
        ...
        public GiaodichwebDb giaodichwebDb;
        public GiaodichwebctDb giaodichwebctDb;
        public GiaodichManage(IConfiguration configuration, MARTDbContext _db) : base(configuration)
        {
            db = _db;
            ...
            giaodichwebDb = new GiaodichwebDb(_db);
            giaodichwebctDb = new GiaodichwebctDb(_db);
        }
        ...
        internal List<Giaodichweb> GetGiaodichwebs(OrderRequest orderRequest)
        {
            DateTime tuNgay = DateTime.Parse(orderRequest.tuNgay);
            DateTime denNgay = DateTime.Parse(orderRequest.denNgay);
            List<Giaodichweb> giaodichwebs = new List<Giaodichweb>();
            giaodichwebs = db.Giaodichwebs.Where(d => d.Trangthai == Int32.Parse(orderRequest.trangThai)
                && d.Ngayphatsinh >= tuNgay
                && d.Ngayphatsinh <= denNgay.AddDays(1)
                && d.Madonvi == this.Madonvi
                && d.Manhanviendathang == orderRequest.maNhanVien).ToList();
            return giaodichwebs;
        }

        internal List<Giaodichweb> GetGiaodichwebsByTrangThai(OrderRequest orderRequest)
        {
            List<Giaodichweb> giaodichwebs = new List<Giaodichweb>();
            giaodichwebs = db.Giaodichwebs.Where(d => d.Trangthai == Int32.Parse(orderRequest.trangThai)).ToList();
            return giaodichwebs;
        }
        ...
        internal List<Giaodichwebct> GetOrderDetails(OrderRequest orderRequest)
        {
            List<Giaodichwebct> giaodichwebcts = new List<Giaodichwebct>();
            giaodichwebcts = db.Giaodichwebcts.Where(d => d.Magiaodichpk == orderRequest.magiaodichpk).ToList();

            return giaodichwebcts;
        }
        ...
        internal object CreateOrder(Order order, string sukien)
        {
            ...
            {
                Khachhang checkkh = db.Khachhangs.Where(d => d.Madonvi == this.Madonvi && d.Dienthoai == order.Khachhang.Dienthoai).FirstOrDefault();
                if (checkkh != null)
                ...
                else
                {
                    ...
                    db.Nhanvienkhachhangs.Add(nhanvienkhachhang);
                    db.SaveChanges();
            ...
            if (result.iSsuccess)
            {
                ...
                if (sukien.Equals("EDIT"))
                    giaodichwebctDb.DeleteByMadonviAndMagiaodichpk(order.Magiaodichpk, order.Madonvi);
                ...
                if (result.iSsuccess)
                {
                    ...
                    else
                    {
                        ...
                        {
                            giaodichwebctDb.DeleteByMadonviAndMagiaodichpk(order.Magiaodichpk, order.Madonvi);
                            giaodichwebDb.DeleteGiaodich(order.Magiaodichpk, order.Madonvi);
                        }
                        return result;
                    }
                }
        ...
        internal orderResult DeleteOrder(Order order)
        {
            try
            {
                ...
                Giaodichweb d = db.Giaodichwebs.Where(d => d.Madonvi == this.Madonvi && d.Magiaodichpk == order.Magiaodichpk).FirstOrDefault();
                if (d != null && d.Trangthai != 10)
                {
                    ...
                    giaodichwebDb.DeleteGiaodich(order.Magiaodichpk, this.Madonvi);
                }
        ...
        private orderResult SaveGiaodichct(List<Orderct> gdchitiet, int sort, Order order)
        {
            try
            {
                ...
                foreach (Orderct rChitiet in gdchitiet)
                {
                    ...
                    try
                    {
                        giaodichwebctDb.InsertQueryGiaodichctBanbuon(order.Magiaodichpk, order.Madonvi, rChitiet.Masieuthi
                            ...
                            rChitiet.Matkno, rChitiet.Matkco,
                            rChitiet.Matknhapkhauno, rChitiet.Matknhapkhauco,
                            rChitiet.Matkthuedacbietno,
                            rChitiet.Matkthuedacbietco,
                            rChitiet.Matkthuegtgtno,
                            rChitiet.Matkthuegtgtco,
                            rChitiet.Matkchietkhauno,
                            rChitiet.Matkchietkhauco,
                            rChitiet.Matkgiamgiano,
                            rChitiet.Matkgiamgiaco,
                            rChitiet.Matklephino,
                            rChitiet.Matklephico,
                            rChitiet.Matkchiphino,
                            rChitiet.Matkchiphico,
                            rChitiet.Matkchiphikhacno,
                            rChitiet.Matkchiphikhacco,
                            rChitiet.Matkkhuyenmaino,
                            rChitiet.Matkkhuyenmaico,
                            rChitiet.Matkthuekhautruno,
                            rChitiet.Matkthuekhautruco,
                            rChitiet.Matkbanamno,
                            rChitiet.Matkbanamco,
                            rChitiet.Matkchietkhausaubanhangno,
                            rChitiet.Matkchietkhausaubanhangco,
                            rChitiet.Matkgiavon,
                            rChitiet.Matkvtu,
                            rChitiet.Iskhuyenmai);
                    }
                    catch (Exception ex)
                    {
            ...
            try
            {
                ...
                if (action == "ADD")
                {
                    try
                    {
                        ...
                        giaodichwebDb.InsertGiaodich(order.Magiaodichpk, order.Madonvi, order.Maptnx, order.Ghichu, 1
                            ...
                            , order.Tendangnhap, order.Tendangnhapsua, order.Nguoigiaohang, order.Ngayphatsinh, order.Maquay, 0);
                    }
                    catch (Exception ex)
                    {
                        result = new orderResult
                ...
                else if (action == "EDIT")
                {
                    try
                    {
                        giaodichwebDb.UpdateGiaodich(order.Maptnx, order.Ghichu, 1
                            , DateTime.Now, order.Sochungtugoc, DateTime.Parse(order.Ngayhoadon.ToString()), "", 0, "", DateTime.Parse(order.Ngaythanhtoan.ToString())
                            , DateTime.Parse(order.Ngaythanhtoan.ToString()), order.Sohoadon
                            , order.Kyhieuhoadon, order.Magiaodichphu, order.Makhachhang, "", "", order.Manhanviendathang
                            , order.Tendangnhap, order.Tendangnhapsua, order.Nguoigiaohang, order.Ngayphatsinh, 0, order.Maquay, order.Magiaodichpk, order.Madonvi);
                    }
                    catch (Exception ex)
                    {
                        result = new orderResult
