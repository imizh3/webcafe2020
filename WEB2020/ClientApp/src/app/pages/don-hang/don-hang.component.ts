...
export class DonHangComponent implements OnInit {
  ...
  load() {
    ...
    orders.subscribe((d) => {
      ...
      this.giaodichs$.forEach((element) => {
        this.tongThanhTien = this.tongThanhTien + parseInt(element.thanhtien);
        this.tongTienCK = this.tongTienCK + parseInt(element.tienck);
        this.tongTienHang = this.tongTienHang + parseInt(element.tienhang);
      });
    });
  }
  btnLocClicked() {
  ...
  luuGiaoDichTrongService(magiaodichpk) {
    ...
    this.dataSourceService.changeGiaoDich(giaoDich);
  }
